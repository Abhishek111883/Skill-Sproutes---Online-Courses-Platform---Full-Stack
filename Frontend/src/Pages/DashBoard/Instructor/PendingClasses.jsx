...
export const PendingClasses = () => {
  ...
  useEffect(() => {
    const fetchclasses = async () => {
      try {
        const response = await axiosSecure.get(`/pending-classes`);
    ...
    <div className="container mx-auto px-4">
      <div className="my-10 mt-20">
        <h1 className="font-bold text-center text-4xl">
          My <span className="text-secondary">Pending</span> Classes
        </h1>
      </div>
      ...
      {instructorclasses.map((item, index) => (
        <div
          key={item._id}
          className="grid grid-cols-1 py-5 md:grid-cols-2 lg:grid-cols-4 gap-x-2 text-center shadow-sm bg-slate-200"
        >
          ...
          <div className="font-semibold text-lg text-rose-600 md:mt-5 lg:mt-0">
            {item.status}
          </div>
